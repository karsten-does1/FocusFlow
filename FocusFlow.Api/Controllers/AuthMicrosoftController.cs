...
namespace FocusFlow.Api.Controllers
{
    ...
    public sealed class AuthMicrosoftController : ControllerBase
    {
            ...
            CancellationToken ct)
        {
            ...
            if (raw == null || string.IsNullOrWhiteSpace(raw.access_token))
            {
                _logger.LogError("Microsoft token response was empty or missing access_token");
                return null;
            }

            var expiresAt = DateTime.UtcNow.AddSeconds(raw.expires_in);
