...
namespace FocusFlow.Core.Domain.Entities
{
    public sealed class Email
    {
        ...
        public string Category { get; private set; } = "Overig";        
        public string SuggestedAction { get; private set; } = "Lezen";
        ...
        public Email(string from, string subject, string body, DateTime receivedUtc, EmailProvider provider = EmailProvider.Unknown, string? externalMessageId = null, Guid? emailAccountId = null)
        {
            ...
            PriorityScore = 0;
            Category = "Nieuw"; 
            SuggestedAction = "Analyseren"; 
            Provider = provider;
            ExternalMessageId = externalMessageId;
            EmailAccountId = emailAccountId;
        }

        public void SetAiAnalysis(int score, string category, string action)
        {
            PriorityScore = Math.Clamp(score, 0, 100);
            Category = category ?? "Overig";
            SuggestedAction = action ?? "Lezen";
        }
